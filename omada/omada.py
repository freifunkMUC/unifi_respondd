...
class Omada:
    ...
    def get_paged(self, path, params={}, data=None, json=None):
        ...
        response = self.session.get(
            self.url_for(path), params=params, data=data, json=json
        )
        response.raise_for_status()
    ...
    def getSiteClients(self, site=None):
        ...
        return self.get_paged(
            f"/sites/{site}/clients", params={"filters.active": "true"}
        )
    ...
    def getSiteSettings(self, site=None):
        ...
        if "beaconControl" in result:
            if self.warnings:
                warnings.warn(
                    "settings['beaconControl'] was removed as it causes an error",
                    stacklevel=2,
                )
